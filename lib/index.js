...
module.exports = function (script, scriptArgs, nodeArgs, opts) {
  ...
  watcher.on('change', function (file) {
    if (notify_list.indexOf(file) === -1) {
      notify_list.push(file);
      notify('Queuing restart', file + ' has been modified');
    }
    if (current_timeout_id) {clearTimeout(current_timeout_id);}
    current_timeout_id = setTimeout(function() { notify_list = []; reload_child() }, cfg.delay * 1000);
  });
  ...
  setTimeout(start, cfg.start_delay * 1000);
};
